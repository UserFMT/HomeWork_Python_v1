...
def test_edit_user():
    ...
    with db.connect() as connection:
        with connection.begin() as transaction:
            try:
                ...
                connection.execute(
                    sql_statement,
                    {'email': email_before, 'user_id': id_user})
                # Фиксируем транзакцию
                transaction.commit()
            except Exception as e:
                # В случае ошибки откатываем транзакцию
    ...
    user_before = db.connect().execute(sql_statement,
                                       {'user_id': id_user}).first()
    assert (user_before.__getattribute__("user_email") !=
            user_after.__getattribute__("user_email"))
