...
def test_create_project(project, headers):
    ...
    del_project = project_api.edit_id_project(headers,
                                              project_new.json()["id"],
                                              project)
...
def test_get_list_project(headers):
    ...
    assert count_after < count_before, \
        "Кол-во проектов не изменилось после добавления."
    project_add = {
        "deleted": True
    }
    del_project = project_api.edit_id_project(headers,
...
def test_edit_id_project():
    ...
    get_project = project_api.get_id_project(headers,
                                             get_project.json()["id"])
    assert get_project.status_code == 200
    assert get_project.json()["id"] is not None
    ...
    project_n = project_api.edit_id_project(headers,
                                            get_project.json()["id"],
                                            project)
